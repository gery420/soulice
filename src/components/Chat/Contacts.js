...
const Contacts = ({ currentChatContact }) => {
  ...
  const getContacts = async () => {
    try {
      setLoad(true);
      let res = await axios.get("http://localhost:3001/common/contacts", {
        ...
        {load ? (
          ...
          contacts
            ...
            .map((contact) => {
              ...
              return (
                <DocChat
                  name={profile.type === "Doctor" ? contact.name : contact.name}
                  image={
                    profile.type === "Doctor"
                      ? contact.profile_pic.image_url
                      : contact.profile_pic.image_url
                  }
                  id={profile.type === "Doctor" ? contact._id : contact._id}
                  currentChatContact={currentChatContact}
                />
              );
            })
