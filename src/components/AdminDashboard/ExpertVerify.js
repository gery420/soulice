...
const ExpertVerify = () => {
  ...
  const fetchExperts = async () => {
    try {
      setLoad(true);
      const res = await axios.get("http://localhost:3001/admin/getAllDoctors", {
  ...
  const verifyDoctor = async (_id) => {
    try {
      ...
      let res = await axios.patch(
        `http://localhost:3001/admin/verifyDoctor/${_id}`,
        {},
        {
          withCredentials: true,
        }
      ...
      if (res.data.success) {
        Swal.fire({
          ...
          text: "This expert can now be seen on the FitHub Portal!",
        });
        fetchExperts();
      }
      setLoad(false);
